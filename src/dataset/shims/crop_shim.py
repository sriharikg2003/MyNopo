...
def get_wavelet_superpixel_representation(images, wavelet='haar', level=1, percentage=10):
    ...
    for b in range(img.shape[0]): 
        ...
        resized_z = np.pad(wavelet_magnitude, ((0, original_img.shape[0] - wavelet_magnitude.shape[0]),
                                                (0, original_img.shape[1] - wavelet_magnitude.shape[1])),
                                                mode='constant')
        ...
        selected_superpixels = np.array(list(sp_cord.keys()))[
            np.array([np.mean(sp_wave_values[k]) for k in sp_wave_values]) < 1
        ]
